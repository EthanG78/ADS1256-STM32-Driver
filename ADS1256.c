...
// NOTE(Ethan): Broken
/**
 *  HAL_StatusTypeDef ADS1256_Read_Continuous(ADS1256 *ads, uint32_t outputArrSize, uint32_t
 * *outputArr)
 *
 ...
 *  in the provided outputArr buffer of size outputArrSize. Once OutputArrSize
 *  conversion results have been read, reading will finish and the function will
 *  return.
 *
 ...
 *  be monitoring the DIN line for the STOPC/RESET command while continuously
 *  converting.
 *
 *  Returns a HAL_StatusTypeDef.
 */
...
ADS1256_Read_Continuous (ADS1256 *ads, uint32_t outputArrSize, uint32_t *outputArr)
{
    ...
    if (ads == NULL || outputArrSize == 0 || outputArr == NULL)
        goto ret_status;

    /// Wait until DRDY is low before we issue the command
    while ((ads->rdyPort->IDR & ads->rdyPin) != 0)
        ;
    ...
    while (sampleIdx < outputArrSize && status == HAL_OK)
    {
        ...
        status = _spi_receive_bytes (ads->spiHandle, 3, inBuffer);
        status = _spi_receive_byte (ads->spiHandle, &inBuffer[0]);
        status = _spi_receive_byte (ads->spiHandle, &inBuffer[1]);
        status = _spi_receive_byte (ads->spiHandle, &inBuffer[2]);
...
// NOTE(Ethan): Broken
/**
 *  HAL_StatusTypeDef ADS1256_Read_Dual_Channel(ADS1256 *ads, ADS1256_Channel channel1, uint32_t
 *      *channel1Output, ADS1256_Channel channel2, uint32_t *channel2Output)
 *
 *  Read the single-ended conversion results (AINn is AINCOM) of two different
 *  channels, channel1 and channel2, and store the results in channel1Output
 *  and channel2Output, respectively. This function follows the procedure for
 *  multiplexer cycling specified on page 21 of the ADS1256 datasheet.
 *
 *  Returns a HAL_StatusTypeDef.
 */
HAL_StatusTypeDef
ADS1256_Read_Dual_Channel (ADS1256 *ads,
                           ...
                           uint32_t *channel2Output)
{
    ...
    if (ads == NULL || ads->mode != MODE_SINGLE_ENDED)
        goto ret_status;
    ...
    // Wait 50 CLKIN periods (assuming CLKIN = 7.68 MHz this would be 6.51 us)
    _delay_us (7);
    status = _spi_receive_bytes (ads->spiHandle, 3, inBuffer); // do I need this?
    status = _spi_receive_byte (ads->spiHandle, &inBuffer[0]);
    status = _spi_receive_byte (ads->spiHandle, &inBuffer[1]);
    status = _spi_receive_byte (ads->spiHandle, &inBuffer[2]);
    ...
    // Convert the 3 bytes captured from the ADS1256 into
    // a single 24 bit value.
    *channel1Output = (((inBuffer[0] & 0x80) ? 0xFF : 0x00) << 24) | ((uint32_t)inBuffer[0] << 16)
                      | ((uint32_t)inBuffer[1] << 8) | inBuffer[2];
    ...
    // Convert the 3 bytes captured from the ADS1256 into
    // a single 24 bit value.
    *channel2Output = (((inBuffer[0] & 0x80) ? 0xFF : 0x00) << 24) | ((uint32_t)inBuffer[0] << 16)
                      | ((uint32_t)inBuffer[1] << 8) | inBuffer[2];
ret_status:
    return status;
}
